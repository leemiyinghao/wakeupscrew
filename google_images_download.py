...
class googleimagesdownload:
    ...
    def _get_one_items(self,page,main_directory,dir_name,limit,arguments):
        items = []
        abs_path = []
        errorCount = 0
        i = 0
        ...
        while count < limit+1:
            object, end_content = self._get_next_item(page)
            if object == "no_links":
                with open("lastest_error_page.html", "w") as err:
                    err.write(bytes(page, "utf-8").decode("unicode_escape"))
                break
            elif object == "":
                page = page[end_content:]
            elif arguments['offset'] and count < int(arguments['offset']):
                ...
                #download the images
                download_message = self.download_image(object['image_thumbnail_url'],object['image_format'],main_directory,dir_name,count,arguments['print_urls'],arguments['socket_timeout'],arguments['prefix'],arguments['print_size'],arguments['no_numbering'],arguments['no_download'],arguments['save_source'],object['image_source'],arguments["silent_mode"],arguments["thumbnail_only"],arguments['format'],arguments['ignore_urls'])
                return download_message, object['image_link']
                if download_status == "success":
    ...
    def download_executor(self,arguments):
        ...
        if arguments['print_paths']:
            print(paths)
        return paths
    def download_to_bytes(self,arguments):
        #for input coming from other python files
        if __name__ != "__main__":
            for arg in args_list:
                if arg not in arguments:
        ...
        for pky in prefix_keywords:
            for sky in suffix_keywords:     # 1.for every suffix keywords
                ...
                while i < len(search_keyword):      # 2.for every main keyword
                    ...
                    self.create_directories(main_directory,dir_name,arguments['thumbnail'], True)     #create directories in OS
                    ...
                    #print("Starting Download...")
                    return self._get_one_items(raw_html,main_directory,dir_name,limit,arguments)    #get all image items and download images
                    paths[pky + search_keyword[i] + sky] = abs_path
